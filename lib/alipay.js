...
function Alipay(opts) {
    ...
    this.rsaPrivate = opts.rsaPrivate;
    this.rsaPublic = opts.rsaPublic;
...
props.preCreate = function (opts) {
    ...
    this.logger.info('[alipay]', 'alipay.trade.precreate req: ' + Object.assign(params, biz_content));
    
    return utl.request({
        method: 'GET',
        url: (this.sandbox? alipay_gate_way_sandbox : alipay_gate_way) + '?' + body
    }).then(res => { this.logger.info('[alipay]', 'alipay.trade.precreate res: ' + res.body); return res });
};
 ...
 * @param {String} opts.subject              商品的标题/交易标题/订单标题/订单关键字等
 * @param {String} opts.amount               订单总金额，单位为元，精确到小数点后两位，取值范围[0.01,100000000]
 * @param {String} opts.auth_code            支付授权码
 * @param {String} [opts.body]               对一笔交易的具体描述信息。如果是多种商品，请将商品描述字符串累加传给body
 * @param {String} [opts.timeout]            设置未付款支付宝交易的超时时间，一旦超时，该笔交易就会自动被关闭。
 *                                           当用户进入支付宝收银台页面（不包括登录页面），会触发即刻创建支付宝交易，此时开始计时。
 *                                           取值范围：1m～15d。m-分钟，h-小时，d-天，1c-当天（1c-当天的情况下，无论交易何时创建，都在0点关闭）。
...
props.microPay = function (opts) {
    ...
    this.logger.info('[alipay]', 'alipay.trade.pay req: ' + JSON.stringify(params));

    return utl.request({
        method: 'GET',
        url: (this.sandbox? alipay_gate_way_sandbox : alipay_gate_way) + '?' + body
    }).then(res => { this.logger.info('[alipay]', 'alipay.trade.pay res: ' + res.body); return res });
};
...
props.query = function (opts) {
    ...
    this.logger.info('[alipay]', 'alipay.trade.query req: ' + JSON.stringify(params));

    return utl.request({
        method: 'GET',
        url: (this.sandbox? alipay_gate_way_sandbox : alipay_gate_way) + '?' + body
    }).then(res => { this.logger.info('[alipay]', 'alipay.trade.query res: ' + res.body); return res });
};
...
props.close = function (opts) {
    ...
    this.logger.info('[alipay]', 'alipay.trade.close req: ' + JSON.stringify(params));

    return utl.request({
        method: 'GET',
        url: (this.sandbox? alipay_gate_way_sandbox : alipay_gate_way) + '?' + body
    }).then(res => { this.logger.info('[alipay]', 'alipay.trade.close res: ' + res.body); return res });
};
...
props.refund  = function (opts) {
    ...
    this.logger.info('[alipay]', 'alipay.trade.refund req: ' + JSON.stringify(params));

    return utl.request({
        method: 'GET',
        url: (this.sandbox? alipay_gate_way_sandbox : alipay_gate_way) + '?' + body
    }).then(res => { this.logger.info('[alipay]', 'alipay.trade.refund res: ' + res.body); return res });
};
...
props.refundQuery = function (opts) {
    ...
    this.logger.info('[alipay]', 'alipay.trade.fastpay.refund.query req: ' + JSON.stringify(params));

    return utl.request({
        method: 'GET',
        url: (this.sandbox? alipay_gate_way_sandbox : alipay_gate_way) + '?' + body
    }).then(res => { this.logger.info('[alipay]', 'alipay.trade.fastpay.refund.query res: ' + res.body); return res });
};
...
props.billDownloadUrlQuery = function (opts) {
    ...
    this.logger.info('[alipay]', 'alipay.data.dataservice.bill.downloadurl.query req: ' + JSON.stringify(params));

    return utl.request({
        method: 'GET',
        url: (this.sandbox? alipay_gate_way_sandbox : alipay_gate_way) + '?' + body
    }).then(res => { this.logger.info('[alipay]', 'alipay.data.dataservice.bill.downloadurl.query res: ' + res.body); return res });
};
